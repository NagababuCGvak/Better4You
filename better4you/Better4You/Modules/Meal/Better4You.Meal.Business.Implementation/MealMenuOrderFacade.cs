...
namespace Better4You.Meal.Business.Implementation
{
    public class MealMenuOrderFacade : IMealMenuOrderFacade
    {
        ...
        //public MealOrderManageView GetCompanyOrder(MealMenuOrderFilterView filter)
        //{

        //    var currentUser = Thread.CurrentPrincipal.AsCurrentUser();

        //    var isSchoolUser = (currentUser.UserTypeId() == (int)UserTypes.School);
        ...
        //    var schoolQuery = Repository.Query<School>().Where(d => d.Id == filter.SchoolId).ToList();

        //    //schoolQuery.Join(Repository.Query<MealMenuOrder>() , d => d.)

        //    //var query1 = Repository.Query<MealMenuOrder>().Where(d => d.School.Id == filter.SchoolId).ToList();
        //    //var query2 = Repository.Query<MealMenuOrderItem>().Where(d => d.MealMenuOrder.Id == filter.MealMenuOrderId).ToList();
        //    //var query3 = Repository.Query<MealMenu>().Where(d => d.Menu.Id == filter.m).ToList();
        //    //var query4 = Repository.Query<Menu>().Where(d => d.Id == fi).ToList();
        ...
        //    var query1 = from schools in Repository.Query<School>()
        ...
        //                where (mm.RecordStatus == (int)RecordStatuses.Active && mm.MealType == filter.MealTypeId &&
        //                    // ReSharper restore ImplicitlyCapturedClosure
        //                    mm.ValidDate >= startedAt && mm.ValidDate <= endedAt && mmoi.RecordStatus == (int)RecordStatuses.Active)
        //                select
        //                  new
        //                  {
        ...
        //                      hasAdditionalFruit = m.AdditionalFruit,
        //                      hasAdditionalVegetable = m.AdditionalVeg
        //                  };
        //    var queryResuls = query1.AsEnumerable().Distinct().ToList();


        //    queryResuls.ForEach(sc =>
        //    {
        //        if(sc.schoolName == filter.schoolName)
        //        {
        //           // var schoolNames = query.Select(s => s.Menu.Schools).ToList();

        //            var schoolTypeMealMenu = queryResuls.Where(d => d.SType != (int)MenuSchoolTypes.None && d.schoolid == filter.SchoolId && d.SType == filter.SchoolType)
        //            .Select(d => new MealMenuOrderItemView
        //            {
        //                MealType = new GeneralItemView { Id = d.mealType },
        //                MealMenuId = d.mealmenuid,
        //                MealMenuValidDate = d.validDate,
        //                MenuId = d.menuID,
        //                MenuName = d.menuName,
        //                MenuType = new GeneralItemView { Id = d.menuTypeId },
        //                HasAdditionalFruit = d.hasAdditionalFruit,
        //                HasAdditionalVegetable = d.hasAdditionalVegetable,
        //            }).AsEnumerable();

        //            var schoolMealMenu = queryResuls.Where(d => d.schoolid == filter.SchoolId)
        //                .Select(d => new MealMenuOrderItemView
        //                {
        //                    MealType = new GeneralItemView { Id = d.mealType },
        //                    MealMenuId = d.mealmenuid,
        ...
        //                    HasAdditionalVegetable = d.hasAdditionalVegetable,
        //                }).AsEnumerable();
        //            var genericMealMenu = queryResuls.Where(d => !d.schoolName.Any())
        //                .Select(d => new MealMenuOrderItemView
        //                {
        //                    MealType = new GeneralItemView { Id = d.mealType },
        //                    MealMenuId = d.mealmenuid,
       ...
       int pageIndex, string orderByField, bool orderByAsc, out int totalCount)
        ...
        public List<DateRangeReportOrderItemView> GetDateRenageOrderItems(DateRangeOrderItemFilterView filter)
        {
            ...
            //var orderItems1 = query.GroupBy(x => x.OrderId);
            //var query1 = query.OrderBy(d => d.SchoolName).OrderByDescending(x => x.OrderId);
